#!/usr/bin/env python3
"""Comprehensive Chinese localization for ALL TrollStore UI strings"""
import sys

subs = [
    # === TrollStore/TSRootViewController.m (Tabs) ===
    ("TrollStore/TSRootViewController.m", [
        ("Apps", "应用"),
        ("Settings", "设置"),
    ]),
    
    # === TrollStore/TSAppTableViewController.m (App list) ===
    ...
    ("TrollStore/TSInstallationController.m", [
        ...
        ("Error downloading app: %@", "下载应用失败: %@"),
        ("Installing ldid", "正在安装 ldid"),
        ("Error", "错误"),
        ("Error downloading ldid: %@", "下载 ldid 失败: %@"),
    ]),
    
    # === TrollStore/TSSettingsListController.m (Settings tab) ===
    ("TrollStore/TSSettingsListController.m", [
        ...
        ("Uninstall Persistence Helper", "卸载持久助手"),
        ...
        ("When iOS rebuilds the icon cache, all TrollStore apps including TrollStore itself will be reverted to \"User\" state and either disappear or no longer launch. The only way to have persistence in a", "当 iOS 重建图标缓存时，所有巨魔商店应用（包括巨魔商店自身）将恢复为"用户"状态，可能消失或无法启动。"),
        ("Helper Installed as Standalone App", "助手已安装为独立应用"),
        ("Helper Installed into %@", "助手已安装到 %@"),
        ("Apps will be registered as User by default since AppSync Unified is installed.", "由于已安装 AppSync Unified，应用将默认注册为用户。"),
        ("Apps will be registered as System by default since AppSync Unified is not installed. When apps loose their System registration and stop working, press \"Refresh App Registrations\" in the Utilities section.", "由于未安装 AppSync Unified，应用将默认注册为系统应用。当应用丢失系统注册而停止工作时，请在工具区域按"刷新应用注册"。"),
        ("Uninstall", "卸载"),
        ("Update TrollStore to %@", "更新巨魔商店到 %@"),
        ("transferApps", "transferApps"),
        ("persistenceHelperInstalled", "persistenceHelperInstalled"),
        ("ldidInstalled", "ldidInstalled"),
        ("ldid.version", "ldid.version"),
        ("You are about to uninstall TrollStore, do you want to preserve the apps installed by it?", "您即将卸载巨魔商店，是否保留已安装的应用？"),
        ("Uninstall TrollStore, Uninstall Apps", "卸载巨魔商店，卸载应用"),
        ("Uninstall TrollStore, Preserve Apps", "卸载巨魔商店，保留应用"),
    ...
    # === TrollHelper/TSHRootViewController.m (PersistenceHelper app) ===
    ("TrollHelper/TSHRootViewController.m", [
        ...
        ("TrollStore", "巨魔商店"),
        ("Refresh App Registrations", "刷新应用注册"),
        ("Register Persistence Helper", "注册持久助手"),
        ("Uninstall Persistence Helper", "卸载持久助手"),
        ...
        ("Install Persistence Helper", "安装持久助手"),
        ("Update TrollStore to %@", "更新巨魔商店到 %@"),
        ("installTrollStore", "installTrollStore"),
        ("uninstallTrollStore", "uninstallTrollStore"),
        ("uninstallPersistenceHelper", "uninstallPersistenceHelper"),
        ("registerPersistenceHelperPressed -> %d", "registerPersistenceHelperPressed -> %d"),
        ("register-user-persistence-helper", "register-user-persistence-helper"),
        ("unregister-user-persistence-helper", "unregister-user-persistence-helper"),
        ("register-standalone-persistence-helper", "register-standalone-persistence-helper"),
        ("unregister-standalone-persistence-helper", "unregister-standalone-persistence-helper"),
        ...
        ("Unregister Persistence Helper", "取消注册持久助手"),
    ]),
...
for fname, reps in subs:
    ...
    for en, cn in reps:
        ...
        total += count
        if count:
            print(f"  [{fname}] {en[:40]} -> {cn[:20]} ({count})", file=sys.stderr)
